...
pub fn main() !void {
    ...
    //json.value.dump();
    const version = json.value.object.get("master").?
        .object.get("version").?
        .string;
    const url = json.value.object.get("master").?
        .object.get("x86_64-windows").?
        .object.get("tarball").?
        .string;
    const size = try std.fmt.parseInt(
        i32,
        json.value.object.get("master").?
            .object.get("x86_64-windows").?
            .object.get("size").?
            .string,
        10,
    ...
    var req_download = try client.request(
        std.http.Method.GET,
        uri_download,
        headers,
        .{},
    ...
    if (zip.zip_stream_open(
        zip_buf.ptr,
        zip_buf.len,
        zip.ZIP_DEFAULT_COMPRESSION_LEVEL,
        'r',
    )) |archive| {
        defer zip.zip_stream_close(archive);

        try std.fs.cwd().deleteTree(output_path);
        ...
        for (0..total) |i| {
            if (zip.zip_entry_openbyindex(archive, @intCast(i)) == 0) {
                ...
                if (res < 0) std.log.err("error on reading: {}", .{res});
            }
        }
    }
}
